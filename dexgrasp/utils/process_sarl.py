...
def process_ppo(args, env, cfg_train, logdir):
    ...
    ppo = PPO(vec_env=env,
              ...
              asymmetric=(env.num_states > 0),
              is_vision=is_vision,
              test_all_object=args.test_all_object,
...
def process_dagger(args, env, cfg_train, logdir):
    ...
    dagger = DAGGER(vec_env=env,
              ...
              expert_chkpt_path = expert_chkpt_path,
              is_vision = is_vision, 
              pointnet_dir=args.pointnet_dir,
              )
...
def process_residual(args, env, cfg_train, logdir):
    from algorithms.rl.residual import Residual, ActorCritic
    learn_cfg = cfg_train["learn"]
    #is_testing = learn_cfg["test"]
    is_testing = args.test
    is_vision = args.vision
    ...
    residual = Residual(vec_env=env,
            ...
            asymmetric=(env.num_states > 0),
            is_vision=is_vision,
            base_model_list_dir=args.base_model_list_dir,
            base_obs_num=args.base_obs_num,
            residual_obs_num=args.residual_obs_num,
            save_test_traj=args.save_test_traj,
            )
    ...
    if is_testing and args.model_dir != "":
        print("Loading model from {}".format(chkpt_path))
        residual.test(chkpt_path)
    elif args.model_dir != "":
        print("Loading model from {}".format(chkpt_path))
        residual.load(chkpt_path)
