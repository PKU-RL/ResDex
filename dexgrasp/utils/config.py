...
def retrieve_cfg(args, use_rlg_config=False):
    ...
    elif args.task == "ShadowHandBlindGrasp":
        return os.path.join(args.logdir, "shadow_hand_blind_grasp/{}/{}".format(args.algo,args.algo)), "cfg/{}/config.yaml".format(args.algo), "cfg/shadow_hand_blind_grasp.yaml"
    elif args.task == "ShadowHandResidualGrasp":
        return os.path.join(args.logdir, "shadow_hand_residual_grasp/{}/{}".format(args.algo,args.algo)), "cfg/{}/config.yaml".format(args.algo), "cfg/shadow_hand_residual_grasp.yaml"
    elif args.task == "ShadowHandPcl":
        return os.path.join(args.logdir, "shadow_hand_pcl/{}/{}".format(args.algo,args.algo)), "cfg/{}/config.yaml".format(args.algo), "cfg/shadow_hand_pcl.yaml"
    elif args.task == "AllegroHandBlindGrasp":
        return os.path.join(args.logdir, "allegro_hand_blind_grasp/{}/{}".format(args.algo,args.algo)), "cfg/{}/config.yaml".format(args.algo), "cfg/allegro_hand_blind_grasp.yaml"
    else:
        warn_task_name()
...
def get_args(benchmark=False, use_rlg_config=False):
    custom_parameters = [
        ...
        {"name": "--datatype", "type": str, "default": "random",
         ...
         {'name': '--base_model_dir', 'type': str, 'default': '', 'help': 'Base model for Residual policy'},
         {'name': '--base_model_list_dir', 'type': str, 'default': '', 'help': 'Base model list for Residual policy'},
         {'name': '--base_obs_num', 'type': int, 'default':0, 'help': 'Number of base observations for Residual policy'},
         {'name': '--residual_obs_num', 'type': int, 'default':0, 'help': 'Number of residual observations for Residual policy'},
         {'name': '--test_all_object', 'action': 'store_true', 'default': False, 'help': 'Test all objects in the training set'},
         {'name': '--save_test_traj', 'action': 'store_true', 'default': False, 'help': 'Save trajectories of test set'},
         {"name": "--pointnet_dir", "type": str, "default": "",
            "help": "Choose a pointnet dir"},
         {'name': '--cluster', 'type': int, 'default':0, 'help': 'choose the directory to save'},
            ]
