...
from utils.pcl import *
from tasks.hand_base.base_task import BaseTask
from isaacgym import gymtorch
from isaacgym import gymapi


class ShadowHandPcl(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless,
        ...
        num_obs += self.points_per_object*3
        self.num_obs_dict = {
            "full_state": num_obs
        }
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # create a pcl buf to store every original pcl for every env
        self.obj_pcl_buff = torch.zeros(self.num_envs, self.max_points_per_object, 3, device=self.device)
        for i in range(self.num_envs):
            ...
            # load the pcl
            pcl_fn = "../assets/pcldata/"+object_code+scale2str[scale]+".npy"
            pcl = torch.tensor(np.load(pcl_fn)).to(self.device)
            self.obj_pcl_buff[i, :, :] = pcl
    ...
    # special design for load YCB dataset
    def _create_envs_YCB(self, num_envs, spacing, num_per_row):
        ...
        # for the vision-based policy, it do not need pc_features
        self.visual_feat_buf = torch.zeros((self.num_envs, 64), device=self.device)
        
        # load YCB objects
        asset_root = "../assets"
        urdf_path = "ycb/urdf_all"
        # show all the file in the asset_root/urdf_path
        # print(os.path.join(asset_root, urdf_path))
        # exit(0)
        obj_fn_list = os.listdir(os.path.join(asset_root, urdf_path))
        self.obj_assets = []
        self.obj_names = []
        for object_fn in obj_fn_list:
            # eliminate the .urdf in the object_fn
            object_name = object_fn.split(".")[0]
            self.obj_names.append(object_name)
            # load manipulated object and goal assets
            object_asset_options = gymapi.AssetOptions()
            object_asset_options.density = 500
            object_asset_options.fix_base_link = False
            ...
            object_asset = self.gym.load_asset(self.sim, asset_root, os.path.join(urdf_path, object_fn), object_asset_options)
            self.obj_assets.append(object_asset)
        ...
        for i in range(self.num_envs):
            ...
            # id = int(i / self.num_envs * len(self.object_code_list))
            id = int(i / self.num_envs * len(self.obj_assets))

            # load the pcl
            # pcl_fn = "../assets/pcldata/"+object_code+scale2str[scale]+".npy"
            pcl_fn = os.path.join(asset_root, "ycb/pointclouds", self.obj_names[id]+".npy")
            pcl = torch.tensor(np.load(pcl_fn)).to(self.device)
            self.obj_pcl_buff[i, :, :] = pcl
             
            # add object
            object_handle = self.gym.create_actor(env_ptr, self.obj_assets[id], object_start_pose, "object", i, 0, 0)
            self.object_init_state.append([object_start_pose.p.x, object_start_pose.p.y, object_start_pose.p.z,
                                           object_start_pose.r.x, object_start_pose.r.y, object_start_pose.r.z,
                                           object_start_pose.r.w,
                                           0, 0, 0, 0, 0, 0])
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        # get the obj pcl
        sampled_pcl = self.transform_obj_pcl_2_world()
        # print("self.obs_buf.shape", self.obs_buf[:, now_position: now_position+self.points_per_object*3].shape)
        # print("sampled_pcl.shape", sampled_pcl.shape)
        # exit()
        self.obs_buf[:, now_position: now_position+self.points_per_object*3] = sampled_pcl.reshape(self.num_envs,-1)
        now_position += self.points_per_object*3
        # should be 512 * 3
    ...
    def transform_obj_pcl_2_world(self):
        ...
        # should we always change the point cloud?
        self.sampled_point_idxs = farthest_point_sample(self.obj_pcl_buff, self.points_per_object, self.device)
        sampled_pcl = index_points(self.obj_pcl_buff, self.sampled_point_idxs, self.device)
