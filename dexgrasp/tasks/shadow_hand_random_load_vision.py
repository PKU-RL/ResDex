...
class ShadowHandRandomLoadVision(BaseTask):
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        self.obs_buf[:, now_position:now_position+ self.num_pc_flatten].copy_(points_fps.reshape(self.num_envs, self.num_pc_flatten))
        now_position += self.num_pc_flatten
        mask_hand = others["mask_hand"]
        mask_object = others["mask_object"]
        self.obs_buf[:, now_position:now_position+self.num_pc_downsample].copy_(mask_hand)
        now_position += self.num_pc_downsample
        self.obs_buf[:, now_position:now_position+self.num_pc_downsample].copy_(mask_object)
        now_position += self.num_pc_downsample
