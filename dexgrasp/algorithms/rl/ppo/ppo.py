...
class PPO:
    ...
    def __init__(self,
                 ...
                 asymmetric=False,
                 is_vision = False,
                 test_all_object = False,
                 cluster=0,
                 ):
        self.is_vision = is_vision
        self.test_all_object = test_all_object
        ...
        # print("self.state_space.shape: ", self.state_space.shape)
    ...
    def run(self, num_learning_iterations, log_interval=1):
        ...
        if self.is_testing and not self.test_all_object:
            for _ in range(200):
                with torch.no_grad():
                    if self.apply_reset:
                        current_obs = self.vec_env.reset()
                ...
                if _ == 200-2:
                    # success_tensor = self.vec_env.task.successes
                    success_rate=self.vec_env.task.successes.sum()/self.vec_env.num_envs
                    print("Success rate: ", success_rate.item())
            exit()
        
        if self.test_all_object and self.is_testing:
            success_tensor = torch.zeros(self.vec_env.num_envs, dtype=torch.float, device=self.device) 
            ...
            for it in range(self.current_learning_iteration, num_learning_iterations):
                ...
                if it % log_interval == 0:
                    if self.cluster == 0:
                        ...
                        self.save(os.path.join(self.log_dir, str(self.cluster)+'/model_{}.pt'.format(it)))
                ep_infos.clear()
    ...
    # a function used to for testing all the objects
    def set_vector_env(self, vec_env):
        self.vec_env = vec_env
