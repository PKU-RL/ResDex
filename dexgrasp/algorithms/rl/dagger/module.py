...
class Actor(nn.Module): # student
    ...
    def __init__(self, obs_shape, states_shape, actions_shape, initial_std, model_cfg, asymmetric=False, use_pc = False, pointnet_dir= None):
        ...
        # 0~147(robot state), 0~153(base model obs), 0~227(residual model obs)
        self.num_obs = 227 
        
        if self.use_pc:
            self.num_obs = 147 # prioception obs
            if self.backbone_type == "pn":
                # the pcl is 512 * 3
                self.backbone = PointNetBackbone(pc_dim=3, feature_dim=128)
    ...
    def act(self, observations):
        if self.use_pc and not self.freeze_backbone:
            if self.backbone_type =="transpn":  
                pc = observations[:, 227:227+6144].reshape(-1, 1024, 6)
                mask = observations[:, 227+6144:].reshape(-1, 1024, 2)
                data = {"pc": pc, "state": observations[:, :147], "mask": mask}
                pc_feature = self.backbone(data)[0].reshape(-1, 128)
            else:
                pc = observations[:, 227:].reshape(-1, 512, 3)
                pc_feature = self.backbone(pc)[0].reshape(-1, 128)
            observations = torch.cat([observations[:, :147], pc_feature], dim=1)
            ...
            with torch.no_grad():
                if self.backbone_type =="transpn":
                    ...
                    pc = observations[:, 227:].reshape(-1, 1024, 8)
                    pc_feature = self.backbone(pc)[0].reshape(-1, 128)
            observations = torch.cat([observations[:, :147], pc_feature], dim=1)
            actions = self.actor(observations)
        else:
            actions = self.actor(observations[:,:227])
        return actions
    ...
    def act_inference(self, observations):
        if self.use_pc and not self.freeze_backbone:
            if self.backbone_type =="transpn":
                ...
                # print("left.shape",observations[:, 227:].shape) 
                pc = observations[:, 227:].reshape(-1, 512, 3)
                pc_feature = self.backbone(pc)[0].reshape(-1, 128)
            observations = torch.cat([observations[:, :147],  pc_feature], dim=1)
            actions = self.actor(observations)
